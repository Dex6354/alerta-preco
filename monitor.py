...
def monitorar_preco():
    ...
    with sync_playwright() as p:
        ...
        try:
            # Tenta carregar apenas o HTML estrutural básico
            page.goto(url, wait_until="commit", timeout=60000)
            
            # Busca o script que contém o JSON com os dados do produto
            ...
            if script_tag.count() > 0:
                ...
                # Se não achar o JSON, tenta buscar direto no HTML usando Regex
                content = page.content()
                match = re.search(r'"lowPrice":\s*([0-9.]+)', content)
                if match:
                    preco = float(match.group(1))
                    print(f"Preço capturado via Regex: R$ {preco}")
                    if preco <= alvo:
                        msg = f"🔥 Alerta! Preço baixou para R$ {preco:.2f}.\nLink: {url}"
                        enviar_telegram(token, chat_id, msg)
                else:
                    raise Exception("Não foi possível localizar o preço no HTML estrutural.")
