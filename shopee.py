...
# ============================================================
# ⚠️ SEUS COOKIES: Cole o conteúdo da linha "Cookie:" aqui dentro
# ============================================================
COOKIES_LOCAL = "csrftoken=w2nwOL9o7nYtJQ2yI8TFF2wVN8zZfSbT;"

# Validação e carregamento dos cookies
if COOKIES_LOCAL.strip():
    SHOPEE_COOKIES = COOKIES_LOCAL.strip()
    print("🍪 STATUS: Usando cookies definidos direto no código.")
else:
    SHOPEE_COOKIES = os.environ.get("SHOPEE_COOKIES", "").strip()
    if SHOPEE_COOKIES:
        print("🌐 STATUS: Usando cookies carregados do GitHub Secrets.")
    else:
        SHOPEE_COOKIES = ""
        print("❌ ERRO: Insira os cookies na variável COOKIES_LOCAL para testar.")
...
def buscar_preco_shopee(url_produto):
    if not SHOPEE_COOKIES:
        raise Exception("Abortando: Não há Cookies válidos configurados.")
    ...
    # INJETADOS OS CODES ATUALIZADOS QUE VOCÊ ENVIOU DO LOG
    headers = {
    ...
    if response.status_code == 403:
        raise Exception("API Shopee retornou status 403 (Sessão inválida ou expirada)")
...
def monitorar_grupo(alvo, nome_item, urls, token, chat_id):
    ...
    for url in urls:
        print(f"🔍 Monitorando: {nome_item} | Alvo: R$ {alvo:.2f}")
        try:
            preco, nome_real, imagem_url = buscar_preco_shopee(url)
            print(f"💰 Atual: R$ {preco:.2f} | 🎯 Alvo: R$ {alvo:.2f}")
            if preco <= alvo:
                atingiram.append({
                    "nome": nome_real,
...
def main():
    if not SHOPEE_COOKIES:
        print("❌ Execução interrompida: Configure a variável COOKIES_LOCAL.")
        sys.exit(1)
