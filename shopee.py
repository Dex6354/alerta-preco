...
# ============================================================
# CONFIGURAÇÕES SHOPEE (ATUALIZE ESTES TOKENS DIARIAMENTE)
# ============================================================
SHOPEE_HEADERS = {
    ...
    # ⚠️ Cole os valores atualizados do seu navegador aqui embaixo:
...
def buscar_preco_shopee(url):
    ...
    # Validação do erro de anti-bot da Shopee
    if response.status_code == 200 and "error" in response.json():
        error_code = response.json().get("error")
        if error_code == 90309999:
            raise Exception("Chaves de segurança/anti-bot da Shopee expiraram! Atualize os Headers.")
